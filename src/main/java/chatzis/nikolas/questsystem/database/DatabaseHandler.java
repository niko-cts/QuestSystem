...
public final class DatabaseHandler {
    ...
    public boolean createTableIfNotExists(String tableName, List<String> columns) {
        ...
        for (int i = 0; i < columns.size(); i++) {
            ...
            if (isInvalidSQLIdentifier(column)) {
                logger.log(Level.WARNING, "Could not create table! Invalid column {0} for table {1}", new Object[]{column, tableName});
                return false;
            }
            sql.append(column);
            if (i < columns.size() - 1) {
        ...
        try (Statement stmt = this.dbConnection.createStatement()) {
            var result = stmt.execute(sql.toString());
