...
public class PlayerInfoDatabase {
	...
	public void updatePlayerDataFromMultiplePlayers(Map<QuestPlayer, Instant> questPlayersWithLogout) {
		...
		for (Map.Entry<QuestPlayer, Instant> entry : questPlayersWithLogout.entrySet()) {
			QuestPlayer player = entry.getKey();
			try {
				player.lockDatabaseUpdate();
				completeSQL.append(getUpdatePlayerSQLStatements(player, entry.getValue()));
				player.databaseUpdatedResetEntries();
			} finally {
				player.unlockDatabaseUpdate();
			}
		}
	...
	public void updateAllPlayerData(QuestPlayer questPlayer, Instant lastLogout) {
		String updatePlayerSQLStatements;
		try {
			questPlayer.lockDatabaseUpdate();
			updatePlayerSQLStatements = getUpdatePlayerSQLStatements(questPlayer, lastLogout);
		} finally {
			questPlayer.unlockDatabaseUpdate();
		}

		if (updatePlayerSQLStatements != null && !this.databaseHandler.execute(updatePlayerSQLStatements))
