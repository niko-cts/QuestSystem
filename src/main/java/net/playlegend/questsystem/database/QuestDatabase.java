...
public class QuestDatabase {
	...
	private static final String TABLE_QUEST_REWARD_MAP = "system_quests_rewards_mapping";
	...
	private QuestDatabase() {
		...
		dbHandler.createTableIfNotExists(TABLE_QUEST_STEPS_INFO, List.of(
				...
				"CONSTRAINT fk_stepForQuestId FOREIGN KEY(quest_id) REFERENCES " + TABLE_QUESTS + "(id) ON DELETE CASCADE")
		);
		...
		dbHandler.createTableIfNotExists(TABLE_QUEST_REWARD_MAP, List.of(
				...
				"CONSTRAINT fk_quest_id FOREIGN KEY (quest_id) REFERENCES " + TABLE_QUESTS + "(id) ON DELETE CASCADE",
				"CONSTRAINT fk_reward_id FOREIGN KEY (reward_id) REFERENCES " + TABLE_QUEST_REWARD_INFO + "(id) ON DELETE CASCADE")
		);
	}
	...
	public void deleteQuest(int id) {
		this.dbHandler.delete(List.of(TABLE_QUESTS),
				List.of("WHERE id=" + id + " LIMIT 1"));
	}
	...
	private boolean insertRewardObjectsAndPutExistingAndRemoveFromRewardObjects(List<QuestReward<?>> currentRewards, Map<Integer, QuestReward<?>> existingRewards) {
		if (currentRewards.isEmpty()) return true;
		...
		try (ResultSet rewardSet = this.dbHandler.select(TABLE_QUEST_REWARD_INFO, List.of("*"),
				...
				+ ") AND reward_object IN(" + currentRewards.stream()
						.map(r -> {
							try {
								return "'" + QuestObjectConverterUtil.convertObjectToDatabaseString(r.getRewardObject()) + "'";
							} catch (IOException exception) {
								QuestSystem.getInstance().getLogger().log(Level.SEVERE, "Could not convert Quest reward to database string!", exception);
								return null;
							}
			...
			while (rewardSet != null && rewardSet.next()) {
				try {
					RewardType type = RewardType.valueOf(rewardSet.getString("type"));
					Object rewardObject = QuestObjectConverterUtil.convertDatabaseStringToQuestRewardObject(rewardSet.getString("reward_object"), type);
					QuestReward<?> existingReward = currentRewards.stream().filter(f -> f.getRewardType() == type
					                                                                    && f.getRewardObject().equals(rewardObject)).findFirst().orElseThrow();
					currentRewards.remove(existingReward);
					existingRewards.put(rewardSet.getInt("id"), existingReward);
				} catch (IOException | ClassNotFoundException e) {
					QuestSystem.getInstance().getLogger().log(Level.SEVERE, "Could not convert stored reward type. This should not be happen!", e);
				} catch (IllegalArgumentException e) {
					QuestSystem.getInstance().getLogger().log(Level.SEVERE, "Could not find given Reward type loaded from database: " + rewardSet.getString("type"));
				} catch (NoSuchElementException e) {
					QuestSystem.getInstance().getLogger().log(Level.SEVERE, "Reward was loaded but could not be find in list. This should not be happen!");
				}
			}
			return true;
