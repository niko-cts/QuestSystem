...
public final class DatabaseHandler {
    ...
    public Integer insertIntoTableAndGetGeneratedKey(String tableName, String primaryColumnKey, List<String> columns, List<Object> values) {
        if (!this.reconnectIfClosed())
            return null;
    ...
    public boolean execute(String sql) {
        ...
        try {
            return this.dbConnection.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE).execute(sql);
        } catch (SQLException e) {
            logger.log(Level.SEVERE, "Could not perform sql:", e);
            logger.log(Level.SEVERE, "SQL-Query: {0}", sql);
            return false;
